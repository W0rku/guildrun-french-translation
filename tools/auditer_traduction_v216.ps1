...
if ([string]::IsNullOrWhiteSpace($BaselineFrenchBundle)) {
    $BaselineFrenchBundle = Join-Path $projectRoot 'payload\localization-string-tables-french(fr)_assets_all.v214.bundle'
}
...
& (Join-Path $PSScriptRoot 'auditer_traduction_v212.ps1') `
    ...
    -CorrectionsFile (Join-Path $projectRoot 'translations\corrections-v2.1.6.fr.json') `
    -ExpectedVersion '2.1.6' `
    -ExpectedCorrections 4 `
    -ExpectedEntries 3923
