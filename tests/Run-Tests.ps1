...
function New-Fixture([string] $Name, [string] $SteamBuildId = '25323618') {
    ...
    $catalogSource = switch ($SteamBuildId) {
        '24551494' { 'catalog.bin.official' }
        '24613101' { 'catalog-24613101.bin.official' }
        '24690909' { 'catalog-24690909.bin.official' }
        '24816645' { 'catalog-24816645.bin.official' }
        '24930839' { 'catalog-24816645.bin.official' }
        '25060342' { 'catalog-25060342.bin.official' }
        '25119884' { 'catalog-25060342.bin.official' }
        '25323618' { 'catalog-25323618.bin.official' }
        default { throw "BuildID de fixture inconnu : $SteamBuildId" }
    }
    Copy-Item -LiteralPath (Join-Path $sourcesRoot $catalogSource) -Destination (Join-Path (Split-Path -Parent $bundleRoot) 'catalog.bin')
    $root
}

function Get-IsolatedPolicy([string] $SteamBuildId) {
    $isolated = $policy | ConvertTo-Json -Depth 8 | ConvertFrom-Json
    $fixtureEnglishHash = $policy.Profiles[5].OriginalEnglishHash
    foreach ($profile in $isolated.Profiles) {
        $profile.OriginalEnglishHash = if ($profile.SteamBuildId -eq $SteamBuildId) { $fixtureEnglishHash } else { '0' * 64 }
    }
    $isolated
}
...
function New-PreviousPayload($PreviousPolicy) {
    ...
    foreach ($profile in $PreviousPolicy.Profiles) {
    ...
    Assert-True ((Get-GuildrunSha256 $french) -eq $PreviousPolicy.Profiles[0].PatchedFrenchHash) 'Bundle French V2.1.1 simule incorrect.'
    $root
}

function Get-PreviousPolicy($BasePolicy) {
    $previous = $BasePolicy | ConvertTo-Json -Depth 8 | ConvertFrom-Json
    $previous.Profiles = @($previous.Profiles | Where-Object {
        -not [string]::IsNullOrWhiteSpace([string]$_.PreviousPatchedFrenchHash) -and
        -not [string]::IsNullOrWhiteSpace([string]$_.PreviousPatchedCatalogHash)
    })
    foreach ($profile in $previous.Profiles) {
        $profile.PatchedFrenchHash = $profile.PreviousPatchedFrenchHash
        $profile.PatchedCatalogHash = $profile.PreviousPatchedCatalogHash
        $profile.PayloadFrenchName = $previous.FrenchBundleName
    }
    $previous.PatchedFrenchHash = $previous.Profiles[1].PatchedFrenchHash
...
function New-V214OfficialBackup([string] $Root, $RegistryState) {
    $backupRoot = Join-Path $Root 'Traduction_FR_V2.1\sauvegarde-locale-24816645'
    New-Item -ItemType Directory -Path $backupRoot -Force | Out-Null
    Copy-Item -LiteralPath (Join-Path $sourcesRoot 'localization-string-tables-french(fr)_assets_all.bundle.official') -Destination (Join-Path $backupRoot 'localization-string-tables-french(fr)_assets_all.bundle.backup')
    Copy-Item -LiteralPath (Join-Path $sourcesRoot 'localization-locales_assets_all.bundle.official') -Destination (Join-Path $backupRoot 'localization-locales_assets_all.bundle.backup')
    Copy-Item -LiteralPath (Join-Path $sourcesRoot 'catalog-24816645.bin.official') -Destination (Join-Path $backupRoot 'catalog.bin.backup')
    [pscustomobject]@{
        FormatVersion = 4
        GameVersion = '0.5.6'
        GameBuild = 792
        SteamBuildId = '24930839'
        ProfileName = 'Steam-24816645-24930839'
        CreatedAtUtc = [DateTime]::UtcNow.ToString('o')
        FrenchSha256 = $policy.OriginalFrenchHash
        LocalesSha256 = $policy.OriginalLocalesHash
        CatalogSha256 = $policy.Profiles[3].OriginalCatalogHash
        LocalePreference = ConvertTo-GuildrunRegistryRecord $RegistryState
    } | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $backupRoot 'manifest.json') -Encoding UTF8
}
...
$legacyFixturePolicy = Get-IsolatedPolicy '24613101'
$previousPolicy = Get-PreviousPolicy $legacyFixturePolicy
$previousPayloadRoot = New-PreviousPayload $previousPolicy

try {
    Invoke-Test 'Version installateur identique : statut Installateur a jour' {
        $json = New-ReleaseJson 'v2.1.8' 'Guildrun_Demo_FR_Installer_V2.1.8.exe' 'https://github.com/W0rku/guildrun-french-translation/releases/download/v2.1.8/Guildrun_Demo_FR_Installer_V2.1.8.exe' ('sha256:' + ('A' * 64)) ''
        [Func[string,string]]$fetch = { param($url) $json }.GetNewClosure()
        $result = [GuildrunFrenchInstallerV21.InstallerUpdateService]::CheckLatestRelease([Version]'2.1.8', $fetch)
    ...
    Invoke-Test 'Release plus recente : version, asset et digest SHA-256 sont exposes' {
        $expected = 'B' * 64
        $json = New-ReleaseJson 'v2.1.9' 'Guildrun_Demo_FR_Installer_V2.1.9.exe' 'https://github.com/W0rku/guildrun-french-translation/releases/download/v2.1.9/Guildrun_Demo_FR_Installer_V2.1.9.exe' ('sha256:' + $expected) ''
        [Func[string,string]]$fetch = { param($url) $json }.GetNewClosure()
        $result = [GuildrunFrenchInstallerV21.InstallerUpdateService]::CheckLatestRelease([Version]'2.1.8', $fetch)
        Assert-True ($result.State -eq [GuildrunFrenchInstallerV21.InstallerUpdateState]::Available) 'Nouvelle Release non detectee.'
        Assert-True ($result.Release.TagName -eq 'v2.1.9' -and $result.Release.AssetName -eq 'Guildrun_Demo_FR_Installer_V2.1.9.exe') 'Release ou asset incorrect.'
    ...
    Invoke-Test 'Un asset hors du depot GitHub attendu est refuse' {
        $json = New-ReleaseJson 'v2.1.8' 'Guildrun_Demo_FR_Installer_V2.1.8.exe' 'https://example.com/Guildrun_Demo_FR_Installer_V2.1.8.exe' $null ''
        [Func[string,string]]$fetch = { param($url) $json }.GetNewClosure()
        $result = [GuildrunFrenchInstallerV21.InstallerUpdateService]::CheckLatestRelease([Version]'2.1.8', $fetch)
    ...
    Invoke-Test 'Le SHA-256 peut etre lu dans les notes si le digest GitHub est absent' {
        $expected = 'C' * 64
        $asset = 'Guildrun_Demo_FR_Installer_V2.1.8.exe'
        $json = New-ReleaseJson 'v2.1.8' $asset "https://github.com/W0rku/guildrun-french-translation/releases/download/v2.1.8/$asset" $null ("SHA-256 $expected  $asset")
    ...
    Invoke-Test 'Une Release sans empreinte reste utilisable avec verification de version interne' {
        $asset = 'Guildrun_Demo_FR_Installer_V2.1.8.exe'
        $json = New-ReleaseJson 'v2.1.8' $asset "https://github.com/W0rku/guildrun-french-translation/releases/download/v2.1.8/$asset" $null ''
        $release = [GuildrunFrenchInstallerV21.InstallerUpdateService]::ParseLatestRelease($json)
    ...
    Invoke-Test 'L identite et la version internes du nouvel EXE correspondent a sa Release' {
        $installerPath = Join-Path $projectRoot 'Installeur\Guildrun_Demo_FR_Installer_V2.1.8.exe'
        if (-not (Test-Path -LiteralPath $installerPath)) { $installerPath = Join-Path $projectRoot 'installer\Guildrun_Demo_FR_Installer_V2.1.8.exe' }
        $hash = [GuildrunFrenchInstallerV21.InstallerUpdateService]::ComputeSha256($installerPath)
        $asset = 'Guildrun_Demo_FR_Installer_V2.1.8.exe'
        $json = New-ReleaseJson 'v2.1.8' $asset "https://github.com/W0rku/guildrun-french-translation/releases/download/v2.1.8/$asset" ('sha256:' + $hash) ''
        $release = [GuildrunFrenchInstallerV21.InstallerUpdateService]::ParseLatestRelease($json)
    ...
    Invoke-Test 'Le Steam BuildID 25323618 officiel est reconnu par ses SHA-256' {
        $root = New-Fixture 'official-current-profile'
        $paths = Get-GuildrunV21Paths -GameRoot $root -PayloadRoot $payloadRoot -Policy $policy
        $state = Get-GuildrunV21State $paths $policy
        Assert-True ($state.Name -eq 'Original' -and $state.SteamBuildId -eq '25323618') 'Profil Steam courant non reconnu.'
    }

    Invoke-Test 'Les six profils Steam sont declares strictement et routes vers le bon French' {
        Assert-True (($policy.Profiles.SteamBuildId -join ',') -eq '24551494,24613101,24690909,24930839,25119884,25323618') 'Liste de profils incorrecte.'
        ...
        Assert-True ($policy.Profiles[2].OriginalEnglishHash -eq 'A39EB85FAE5C1EBBF6385D9D7798E6D8F8287D55402098FDF5AB96D288ACCA8F') 'Hash English 0.5.4 incorrect.'
        Assert-True ($policy.Profiles[2].OriginalCatalogHash -eq '647051CA4D8AAF4ED9E2BB13674E690333C689D9743F88D0DFB4DE3097FA820C') 'Hash catalogue officiel 0.5.4 incorrect.'
        Assert-True ($policy.Profiles[0].GameVersion -eq '0.5.3' -and $policy.Profiles[0].GameBuild -eq 748 -and $policy.Profiles[1].GameVersion -eq '0.5.3' -and $policy.Profiles[1].GameBuild -eq 748) 'Profils 0.5.3 alteres.'
        Assert-True ($policy.Profiles[2].GameVersion -eq '0.5.4' -and $policy.Profiles[2].GameBuild -eq 767) 'Version/build 0.5.4 incorrects.'
        Assert-True ($policy.Profiles[0].PayloadFrenchName -eq $policy.Profiles[1].PayloadFrenchName -and $policy.Profiles[0].PatchedFrenchHash -eq '7C8D467B719EEEE955C0226248EC90004277842B5017436607E7A01EAE388305') 'French V2.1.2 legacy non conserve.'
        Assert-True ($policy.Profiles[2].PayloadFrenchName -eq 'localization-string-tables-french(fr)_assets_all.v213.bundle' -and $policy.Profiles[2].PatchedFrenchHash -eq '07995AA60F88CCAE1FDE1FA375819099906BC0EBCAA0B424358637D00AADDE73') 'French V2.1.3 legacy incorrect.'
        Assert-True ($policy.Profiles[3].OriginalEnglishHash -eq 'CA2B6A9BCEFBC64D44FEFE5B10C5FA77419C5081095584B7B8516C0EC82811BE') 'Hash English 0.5.5 incorrect.'
        Assert-True ($policy.Profiles[3].OriginalCatalogHash -eq 'C48AAD223DB7A7DC3620CEBE29E8AF4C8F0B15990549B32A966DA48BF712F2BF') 'Hash catalogue officiel 0.5.5 incorrect.'
        Assert-True ($policy.Profiles[3].GameVersion -eq '0.5.6' -and $policy.Profiles[3].GameBuild -eq 792) 'Version/build 0.5.6 incorrects.'
        Assert-True (($policy.Profiles[3].CompatibleSteamBuildIds -join ',') -eq '24816645,24930839') 'Alias BuildID 0.5.5/0.5.6 incorrects.'
        Assert-True ($policy.Profiles[3].PayloadFrenchName -eq 'localization-string-tables-french(fr)_assets_all.v214.bundle' -and $policy.Profiles[3].PatchedFrenchHash -eq '907B489269EFD5F359C456CA62EC9FB5C77621B8CCB17AE1122F64F9434D321B') 'French V2.1.4 legacy incorrect.'
        Assert-True ($policy.Profiles[4].OriginalEnglishHash -eq 'C5CF24D3B0A465F77DA89353998A87F5AEAA6A4B2F02BAD8F057AF39D65D472E') 'Hash English 0.5.7 incorrect.'
        Assert-True ($policy.Profiles[4].OriginalCatalogHash -eq '80A47C9F8DC2CB85F7DAD8A25E48A9A9CC25397D3616CFB6F206398CD6C0762A') 'Hash catalogue officiel 0.5.7 incorrect.'
        Assert-True ($policy.Profiles[4].GameVersion -eq '0.5.7' -and $policy.Profiles[4].GameBuild -eq 824) 'Version/build 0.5.7 incorrects.'
        Assert-True (($policy.Profiles[4].CompatibleSteamBuildIds -join ',') -eq '25060342,25119884' -and ($policy.Profiles[4].CompatibleGameBuilds -join ',') -eq '814,824') 'Alias 0.5.7 incorrects.'
        Assert-True ($policy.Profiles[4].PayloadFrenchName -eq $policy.FrenchBundleName -and $policy.Profiles[4].PatchedFrenchHash -eq '4066E7A564AEFE6EC0BBAD17304CF24BA3AA231FEE03E9BDE80A22AAAD1652D0') 'French V2.1.6 courant incorrect.'
    ...
    Invoke-Test 'L audit V2.1.8 des 3923 cles French ne trouve plus de cle, format ou balise divergente' {
        $root = New-Fixture 'translation-audit'
        $paths = Get-GuildrunV21Paths $root $payloadRoot $policy
        & (Join-Path $projectRoot 'tools\auditer_traduction_v218.ps1') -EnglishBundle $paths.English | Out-Null
    }

    Invoke-Test 'Le nouveau profil 0.5.9 utilise exclusivement la correction V2.1.8' {
        $p = $policy.Profiles[5]
        Assert-True ($p.GameVersion -eq '0.5.9' -and $p.GameBuild -eq 842) 'Version/build incorrects.'
        Assert-True ($p.OriginalEnglishHash -eq 'EEA30D7E72CA5B0DB8479648B0F15FD03EA956357E06B8E265E12B21E85ADF31') 'English officiel incorrect.'
        $definition = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $projectRoot 'translations\corrections-v2.1.8.fr.json') | ConvertFrom-Json
        Assert-True ($definition.version -eq '2.1.8' -and @($definition.corrections).Count -eq 1 -and $definition.corrections[0].id -eq '21745596575236113') 'Correction relique incorrecte.'
        Assert-True ($p.PayloadFrenchName -eq 'localization-string-tables-french(fr)_assets_all.v218.bundle') 'French historique reutilise sur 0.5.9.'
    }

    Invoke-Test 'Migration Steam sans sauvegarde reconnue : refus sans ecriture' {
        $root = New-Fixture 'migration-no-backup'
        $paths = Get-GuildrunV21Paths $root $payloadRoot $policy
        Copy-Item -LiteralPath (Join-Path $payloadRoot 'localization-string-tables-french(fr)_assets_all.v214.bundle') -Destination $paths.French -Force
        Copy-Item -LiteralPath (Join-Path $payloadRoot $policy.LocalesBundleName) -Destination $paths.Locales -Force
        $before = Get-ContentHashes $root
        $registry = New-MockRegistry $true 'String' 'de-DE'
        $thrown = $false
        try { Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer | Out-Null } catch { $thrown = $true }
        Assert-True $thrown 'Migration sans sauvegarde acceptee.'
        $after = Get-ContentHashes $root
        foreach ($name in $before.Keys) { Assert-True ($before[$name] -eq $after[$name]) "$name modifie sans sauvegarde." }
        Assert-True ($registry.Store.Value -ceq 'de-DE') 'Preference modifiee sans sauvegarde.'
    }

    Invoke-Test 'Rollback migration 0.5.9 : restaure exactement l etat mixte Steam' {
        $root = New-Fixture 'migration-rollback'
        $paths = Get-GuildrunV21Paths $root $payloadRoot $policy
        Copy-Item -LiteralPath (Join-Path $payloadRoot 'localization-string-tables-french(fr)_assets_all.v214.bundle') -Destination $paths.French -Force
        Copy-Item -LiteralPath (Join-Path $payloadRoot $policy.LocalesBundleName) -Destination $paths.Locales -Force
        New-V214OfficialBackup $root ([pscustomobject]@{ Exists = $true; Kind = 'String'; Value = 'en' })
        $before = Get-ContentHashes $root
        $registry = New-MockRegistry $false $null $null
        $thrown = $false
        try { Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer -FailureInjector { param($stage) if ($stage -eq 'AfterLocale') { throw 'migration failure' } } | Out-Null } catch { $thrown = $true }
        Assert-True $thrown 'Echec migration non propage.'
        $after = Get-ContentHashes $root
        foreach ($name in $before.Keys) { Assert-True ($before[$name] -eq $after[$name]) "$name non restaure apres migration." }
        Assert-True (-not $registry.Store.Exists) 'Absence preference non restauree.'
    }

    Invoke-Test 'Le manifeste V2.1.6 declare exactement quatre nouvelles cles UI uniques' {
        $definition = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $projectRoot 'translations\corrections-v2.1.6.fr.json') | ConvertFrom-Json
        $keys = @($definition.corrections | ForEach-Object { "$($_.table):$($_.id)" })
        Assert-True ($definition.version -eq '2.1.6' -and $definition.baselineSha256 -eq '907B489269EFD5F359C456CA62EC9FB5C77621B8CCB17AE1122F64F9434D321B') 'Entete du manifeste V2.1.6 incorrect.'
        Assert-True ($keys.Count -eq 4 -and @($keys | Select-Object -Unique).Count -eq 4) 'Corrections V2.1.6 absentes ou dupliquees.'
        Assert-True (@($definition.corrections | Where-Object { [bool]$_.add }).Count -eq 4) 'Les quatre nouvelles cles 0.5.7 doivent etre ajoutees.'
    }
    ...
    Invoke-Test 'Les six catalogues chargent les CRC du French propre au profil et des Locales V2.1' {
        $localesCrc = Get-BundleInternalCrc (Join-Path $payloadRoot $policy.LocalesBundleName)
        foreach ($profile in $policy.Profiles) {
            $frenchPath = Join-Path $payloadRoot $profile.PayloadFrenchName
            $frenchCrc = Get-BundleInternalCrc $frenchPath
            ...
            Assert-True ((Get-GuildrunSha256 $frenchPath) -eq $profile.PatchedFrenchHash) "Hash French incorrect pour $($profile.SteamBuildId)."
        }
    }
    ...
    Invoke-Test 'Chaque catalogue officiel ne change qu aux huit octets CRC necessaires' {
        ...
        $v213Diff = @(Get-ByteDiffOffsets (Join-Path $sourcesRoot 'catalog-24690909.bin.official') (Join-Path $payloadRoot 'catalog-24690909.bin'))
        $v214Diff = @(Get-ByteDiffOffsets (Join-Path $sourcesRoot 'catalog-24816645.bin.official') (Join-Path $payloadRoot 'catalog-24816645.bin'))
        $v216Diff = @(Get-ByteDiffOffsets (Join-Path $sourcesRoot 'catalog-25060342.bin.official') (Join-Path $payloadRoot 'catalog-25060342.bin'))
        $v218Diff = @(Get-ByteDiffOffsets (Join-Path $sourcesRoot 'catalog-25323618.bin.official') (Join-Path $payloadRoot 'catalog-25323618.bin'))
        Assert-True (($v218Diff -join ',') -eq '4723,4724,4725,4726,6143,6144,6145,6146') 'Diff catalogue 0.5.9 incorrect.'
        ...
        Assert-True (($v213Diff -join ',') -eq $expected) "Diff 0.5.4 incorrect: $($v213Diff -join ',')"
        Assert-True (($v214Diff -join ',') -eq $expected) "Diff 0.5.5 incorrect: $($v214Diff -join ',')"
        Assert-True (($v216Diff -join ',') -eq $expected) "Diff 0.5.7 incorrect: $($v216Diff -join ',')"
    }

    Invoke-Test 'L installateur V2.1.8 embarque exactement les douze payloads verifies' {
        $installerDirectory = if (Test-Path -LiteralPath (Join-Path $projectRoot 'Installeur')) { 'Installeur' } else { 'installer' }
        $installerPath = Join-Path $projectRoot (Join-Path $installerDirectory 'Guildrun_Demo_FR_Installer_V2.1.8.exe')
        Assert-True (Test-Path -LiteralPath $installerPath -PathType Leaf) 'Installateur V2.1.8 non compile.'
        $assembly = [Reflection.Assembly]::LoadFile([IO.Path]::GetFullPath($installerPath))
        $expectedResources = [ordered]@{
            'GuildrunFRV21.FrenchV218' = $policy.Profiles[5].PatchedFrenchHash
            'GuildrunFRV21.Catalog25323618' = $policy.Profiles[5].PatchedCatalogHash
            'GuildrunFRV21.FrenchCurrent' = $policy.Profiles[4].PatchedFrenchHash
            'GuildrunFRV21.FrenchV214' = $policy.Profiles[3].PatchedFrenchHash
            'GuildrunFRV21.FrenchV213' = $policy.Profiles[2].PatchedFrenchHash
            'GuildrunFRV21.FrenchLegacy' = $policy.Profiles[0].PatchedFrenchHash
            'GuildrunFRV21.Locales' = $policy.PatchedLocalesHash
            'GuildrunFRV21.Catalog24551494' = $policy.Profiles[0].PatchedCatalogHash
            'GuildrunFRV21.Catalog24613101' = $policy.Profiles[1].PatchedCatalogHash
            'GuildrunFRV21.Catalog24690909' = $policy.Profiles[2].PatchedCatalogHash
            'GuildrunFRV21.Catalog24816645' = $policy.Profiles[3].PatchedCatalogHash
            'GuildrunFRV21.Catalog25060342' = $policy.Profiles[4].PatchedCatalogHash
        }
    ...
    Invoke-Test 'Le profil historique selectionne son propre catalogue patche' {
        ...
        $legacyPolicy.Profiles[2].OriginalEnglishHash = ('0' * 64)
        $legacyPolicy.Profiles[3].OriginalEnglishHash = ('0' * 64)
        $legacyPolicy.Profiles[4].OriginalEnglishHash = ('0' * 64)
        $legacyPolicy.Profiles[5].OriginalEnglishHash = ('0' * 64)
        ...
        Assert-True ((Get-GuildrunSha256 $paths.French) -eq $legacyPolicy.Profiles[0].PatchedFrenchHash) 'French V2.1.2 historique non installe.'
    }
    ...
    Invoke-Test 'La sauvegarde locale contient les trois originaux exacts' {
        ...
        Assert-True ($backup.Manifest.GameVersion -eq '0.5.9' -and $backup.Manifest.GameBuild -eq 842 -and [string]$backup.Manifest.SteamBuildId -eq '25323618') 'Profil 0.5.9 build 842 absent du manifeste de sauvegarde.'
    }

    Invoke-Test 'La sauvegarde V2.1.4 BuildID 24816645 reste acceptee sur le BuildID 24930839' {
        $root = New-Fixture 'v214-compatible-backup' '24930839'
        $v214Policy = Get-IsolatedPolicy '24930839'
        $registry = New-MockRegistry $true 'Binary' ([byte[]](102, 114, 0))
        Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -Policy $v214Policy -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer | Out-Null
        $paths = Get-GuildrunV21Paths $root $payloadRoot $v214Policy
        $paths = Set-GuildrunV21ProfilePaths $paths $v214Policy.Profiles[3]
        $backup = Read-GuildrunPersistentBackup $paths
        $backup.Manifest.SteamBuildId = '24816645'
        Assert-GuildrunOriginalPersistentBackup -Backup $backup -Policy $v214Policy -Profile $v214Policy.Profiles[3]
    }

    Invoke-Test 'Une installation deja presente recree exactement la preference fr si Steam l a supprimee' {
        $installRegistry.Store.Exists = $false
        $installRegistry.Store.Kind = $null
        $installRegistry.Store.Value = $null
        $result = Invoke-GuildrunV21Install -GameRoot $installRoot -PayloadRoot $payloadRoot -RegistryReader $installRegistry.Reader -RegistryWriter $installRegistry.Writer -RegistryRestorer $installRegistry.Restorer
        Assert-True ($result.State -eq 'AlreadyInstalled') 'Installation existante non reconnue.'
        Assert-True ($installRegistry.Store.Exists -and $installRegistry.Store.Kind -eq 'Binary') 'Preference fr non recreee.'
        Assert-True (([BitConverter]::ToString([byte[]]$installRegistry.Store.Value)) -eq '66-72-00') 'Contenu fr recree incorrect.'
    }
    ...
    Invoke-Test 'Une installation V2.1.1 complete est reconnue comme mise a niveau autorisee' {
        $script:upgradeRoot = New-Fixture 'upgrade-v211-v213' '24613101'
        ...
        $state = Get-GuildrunV21State $paths $legacyFixturePolicy
    ...
    Invoke-Test 'La mise a niveau V2.1.1 via V2.1.3 conserve la sauvegarde originale exacte' {
        $beforeEnglish = (Get-ContentHashes $script:upgradeRoot).English
        $result = Invoke-GuildrunV21Install -GameRoot $script:upgradeRoot -PayloadRoot $payloadRoot -Policy $legacyFixturePolicy -RegistryReader $script:upgradeRegistry.Reader -RegistryWriter $script:upgradeRegistry.Writer -RegistryRestorer $script:upgradeRegistry.Restorer
        Assert-True ($result.State -eq 'Upgraded') 'Etat de mise a niveau inattendu.'
        $after = Get-ContentHashes $script:upgradeRoot
        Assert-True ($after.French -eq $legacyFixturePolicy.Profiles[1].PatchedFrenchHash -and $after.Locales -eq $legacyFixturePolicy.PatchedLocalesHash -and $after.Catalog -eq $legacyFixturePolicy.Profiles[1].PatchedCatalogHash) 'Triplet 0.5.3 incomplet apres mise a niveau.'
    ...
    Invoke-Test 'Apres mise a niveau, Restaurer remet les officiels et la preference anterieure a V2.1.1' {
        Invoke-GuildrunV21Restore -GameRoot $script:upgradeRoot -PayloadRoot $payloadRoot -Policy $legacyFixturePolicy -RegistryReader $script:upgradeRegistry.Reader -RegistryRestorer $script:upgradeRegistry.Restorer | Out-Null
    ...
    Invoke-Test 'Un echec de mise a niveau restaure exactement V2.1.1 et conserve sa sauvegarde' {
        $root = New-Fixture 'upgrade-rollback' '24613101'
        ...
        $thrown = $false
        try {
            Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -Policy $legacyFixturePolicy -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer -FailureInjector { param($stage) if ($stage -eq 'AfterCatalog') { throw 'echec upgrade simule' } } | Out-Null
        }
        catch { $thrown = $true }
    ...
    Invoke-Test 'Un bundle English inconnu est refuse sans aucune ecriture' {
        $root = New-Fixture 'unknown-english'
        $paths = Get-GuildrunV21Paths $root $payloadRoot $policy
        $bytes = [IO.File]::ReadAllBytes($paths.English); $bytes[0] = $bytes[0] -bxor 1; [IO.File]::WriteAllBytes($paths.English, $bytes)
        $before = Get-ContentHashes $root
        $registry = New-MockRegistry $true 'String' 'de-DE'
        $thrown = $false
        try { Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer | Out-Null } catch { $thrown = $true }
        Assert-True $thrown 'Bundle English inconnu accepte.'
        $after = Get-ContentHashes $root
        foreach ($name in $before.Keys) { Assert-True ($after[$name] -eq $before[$name]) "$name ecrit avec un English inconnu." }
        Assert-True ($registry.Store.Exists -and $registry.Store.Kind -eq 'String' -and $registry.Store.Value -ceq 'de-DE') 'Preference modifiee avec un English inconnu.'
        Assert-True (-not (Test-Path -LiteralPath (Join-Path $root 'Traduction_FR_V2.1'))) 'Sauvegarde creee avec un English inconnu.'
    }
    ...
    Invoke-Test 'L etat laisse par Steam 0.5.9 migre avec une sauvegarde officielle valide et reste restaurable' {
        $root = New-Fixture 'partial-after-steam-update'
        $paths = Get-GuildrunV21Paths $root $payloadRoot $policy
        Copy-Item -LiteralPath (Join-Path $payloadRoot 'localization-string-tables-french(fr)_assets_all.v214.bundle') -Destination $paths.French -Force
        Copy-Item -LiteralPath (Join-Path $payloadRoot $policy.LocalesBundleName) -Destination $paths.Locales -Force
        $previousPreference = [pscustomobject]@{ Exists = $true; Kind = 'Binary'; Value = [byte[]](101, 110, 0) }
        New-V214OfficialBackup $root $previousPreference
        $registry = New-MockRegistry $false $null $null
        $result = Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer
        Assert-True ($result.State -eq 'Upgraded') 'Etat partiel Steam reconnu mais non migre.'
        $installed = Get-ContentHashes $root
        Assert-True ($installed.French -eq $policy.PatchedFrenchHash -and $installed.Locales -eq $policy.PatchedLocalesHash -and $installed.Catalog -eq $policy.PatchedCatalogHash) 'Triplet V2.1.8 incomplet apres migration Steam.'
        Invoke-GuildrunV21Restore -GameRoot $root -PayloadRoot $payloadRoot -RegistryReader $registry.Reader -RegistryRestorer $registry.Restorer | Out-Null
        $restored = Get-ContentHashes $root
        Assert-True ($restored.French -eq $policy.OriginalFrenchHash -and $restored.Locales -eq $policy.OriginalLocalesHash -and $restored.Catalog -eq $policy.OriginalCatalogHash) 'Restauration officielle 0.5.7 incorrecte apres migration.'
        Assert-True ($registry.Store.Exists -and ([BitConverter]::ToString([byte[]]$registry.Store.Value)) -eq '65-6E-00') 'Preference precedant V2.1.4 non restauree apres migration.'
    }

    Invoke-Test 'Une mise a jour Steam sur V2.1.6 reutilise sa sauvegarde exacte et reactive fr' {
        $root = New-Fixture 'steam-update-after-v216' '25119884'
        $previousV216Policy = Get-IsolatedPolicy '25119884'
        $registry = New-MockRegistry $true 'String' 'de-DE'
        Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -Policy $previousV216Policy -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer | Out-Null
        $paths = Get-GuildrunV21Paths $root $payloadRoot $policy
        $paths = Set-GuildrunV21ProfilePaths $paths $previousV216Policy.Profiles[4]
        $manifestPath = Join-Path $paths.BackupRoot 'manifest.json'
        $manifestHash = Get-GuildrunSha256 $manifestPath
        Copy-Item -LiteralPath (Join-Path $sourcesRoot 'catalog-25323618.bin.official') -Destination $paths.Catalog -Force
        $registry.Store.Exists = $false
        $registry.Store.Kind = $null
        $registry.Store.Value = $null
        $result = Invoke-GuildrunV21Install -GameRoot $root -PayloadRoot $payloadRoot -RegistryReader $registry.Reader -RegistryWriter $registry.Writer -RegistryRestorer $registry.Restorer
        Assert-True ($result.State -eq 'Upgraded') 'Etat V2.1.6 apres mise a jour Steam non repris.'
        Assert-True ((Get-GuildrunSha256 $manifestPath) -eq $manifestHash) 'Sauvegarde V2.1.6 reecrite pendant la reprise.'
        Assert-True ($registry.Store.Exists -and ([BitConverter]::ToString([byte[]]$registry.Store.Value)) -eq '66-72-00') 'Preference fr non reactivee.'
        Invoke-GuildrunV21Restore -GameRoot $root -PayloadRoot $payloadRoot -RegistryReader $registry.Reader -RegistryRestorer $registry.Restorer | Out-Null
        $restored = Get-ContentHashes $root
        Assert-True ($restored.French -eq $policy.OriginalFrenchHash -and $restored.Locales -eq $policy.OriginalLocalesHash -and $restored.Catalog -eq $policy.OriginalCatalogHash) 'Restauration officielle apres reprise V2.1.6 incorrecte.'
        Assert-True ($registry.Store.Exists -and $registry.Store.Kind -eq 'String' -and $registry.Store.Value -ceq 'de-DE') 'Preference anterieure V2.1.6 non restauree.'
    }
}
finally {
